...
namespace Custom\Blueprint\Generators;
...
class ModelGenerator implements Generator
{
    ...
    protected function populateStub(string $stub, Model $model)
    {
        if (Blueprint::isLaravel8OrHigher()) {
            ...
            $stub = str_replace(
                PHP_EOL . 'class {{ class }}',
                $this->buildClassPhpDoc($model) . PHP_EOL . 'class {{ class }}',
                $stub,
            );
    ...
    protected function buildClassPhpDoc(Model $model)
    {
        ...
        foreach ($model->columns() as $column) {
            if ($column->dataType() === 'morphs') {
                ...
                $phpDoc .= PHP_EOL;
                $phpDoc .=
                    ' * @property string|null $' . $column->name() . '_type';
                $phpDoc .= PHP_EOL;
                ...
                $phpDoc .=
                    ' * @property string|null $' . $column->name() . '_id';
                $phpDoc .= PHP_EOL;
                $phpDoc .=
                    ' * @property string|null $' . $column->name() . '_type';
                $phpDoc .= PHP_EOL;
    ...
    protected function buildProperties(Model $model)
    {
        ...
        if (config('blueprint.use_guarded')) {
            ...
            if (!empty($columns)) {
                $properties .=
                    ...
                    str_replace(
                        '[]',
                        $this->pretty_print_array($columns, false),
        ...
        if (!empty($columns)) {
            $properties .=
                ...
                str_replace(
                    '[]',
                    $this->pretty_print_array($columns, false),
                    $this->files->stub('model.hidden.stub'),
                );
        }
        ...
        if (!empty($columns)) {
            $properties .=
                ...
                str_replace(
                    '[]',
                    $this->pretty_print_array($columns),
        ...
        foreach ($model->relationships() as $type => $references) {
            foreach ($references as $reference) {
                ...
                if (Str::contains($reference, ':')) {
                    [$foreign_reference, $column_name] = explode(
                        ':',
                        $reference,
                    );
                ...
                if ($type === 'morphTo') {
                    ...
                    $relation =
                        Str::lower(Str::singular($column_name)) . 'able';
                ...
                if ($type === 'morphTo') {
                    ...
                    in_array($type, ['hasMany', 'belongsToMany', 'morphMany'])
                ) {
    ...
    protected function getPath(Model $model)
    {
        $path = str_replace(
            '\\',
            '/',
            Blueprint::relativeNamespace($model->fullyQualifiedClassName()),
        );
    ...
    protected function addTraits(Model $model, $stub)
    {
        ...
        if (Blueprint::isLaravel8OrHigher()) {
            ...
            $stub = Str::replaceFirst(
                '{',
                '{' . PHP_EOL . '    use SoftDeletes;' . PHP_EOL,
                $stub,
            );
        }
    ...
    private function castForColumn(Column $column)
    {
        ...
        if (
            stripos($column->dataType(), 'integer') ||
            $column->dataType() === 'id'
        ) {
            return 'integer';
        }
